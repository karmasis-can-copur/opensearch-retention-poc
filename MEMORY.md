...
Current Dataskope target architecture is Hot + Searchable Snapshot, without a default cold tier:
...
- ISM handles read-only, force-merge, and snapshot for indexes that leave the hot window.
- Target for OpenSearch 3.x is native `convert_index_to_remote` after snapshot, with no separate lifecycle scheduler/controller, but this requires a real remote snapshot repository.
- Local FS snapshot repository remains useful for snapshot validation and manual restore tests; OpenSearch 3.6.0 FS repo did not complete native `convert_index_to_remote` in testing.
- Cold tier remains optional and should only be reintroduced if we need a lower-latency intermediate tier or a relocation/merge buffer.

Current hot+snapshot PoC files:

- `opensearch/lifecycle/dataskope-ism-policy.hot-snapshot.poc.json`
- `opensearch/lifecycle/dataskope-ism-policy.hot-snapshot.native-convert.example.json`
- `docker-compose.hot-snapshot.yml`
- `scripts/promote-hot-snapshot-by-date.ps1`
- `scripts/complete-searchable-snapshot.ps1`
- `scripts/prepare-hot-snapshot-samples.ps1`
- `docs/hot-searchable-snapshot-5k-report-2026-05-06.md`
...
- Current policy model uses a single `events-hot-snapshot` policy with `min_index_age=10d`; old `events-hot-snapshot-after-Nd` policy variants were removed from the live cluster.
- `events-hot-snapshot` has an `ism_template` for `events_*`, and `convert_index_to_remote` now renames searchable snapshot indexes to `remote_$1` so they do not match the hot source pattern.
...
- OpenSearch 3.6.0 was accepted as target and is now used in compose. MinIO/S3-compatible local repo was explicitly rejected by the user, so the PoC is configured with local FS snapshot repository.
...
## Previous Hot/Cold/Search Architecture
...
- Snapshot repo: `dataskope_lifecycle_repo`, path `/mnt/snapshots`.
...
- Existing sample events are dated `2026-04-23`, so they write to `events_2026_04_23`.
- OpenSearch ISM `min_index_age` uses index creation time, not date encoded in index name.
- Backfill/old event-date indexes need explicit promotion with `scripts/promote-events-by-date.ps1`.
...
Policy id: `events-hot-cold`.

File: `opensearch/lifecycle/dataskope-ism-policy.poc.json`.

State flow:

- `hot`: priority 100, transition to cold after `min_index_age=2d`.
- `cold`: read_only, force_merge max_num_segments=1, allocation require `temp=cold`, priority 50.
- `snapshot_ready`: snapshot to `dataskope_lifecycle_repo`, transition after cold/index age reaches `30d`.

`bootstrap-lifecycle.ps1` sets ISM job interval to 1 minute for PoC observation unless `-KeepDefaultIsmInterval` is used.

## Important Scripts

- `scripts/start-lifecycle-poc.ps1`: start cluster, bootstrap lifecycle, optionally run bounded producer.
- `scripts/bootstrap-lifecycle.ps1`: wait for cluster/node count, register snapshot repo, upsert ISM policy, put index template.
- `scripts/lifecycle-list.ps1`: cluster, node, index, shard, repo, snapshot overview.
- `scripts/lifecycle-explain.ps1`: ISM explain as JSON.
- `scripts/promote-events-by-date.ps1`: promotes old calendar-date indexes to cold/snapshot states because ISM age is creation-age based.
- `scripts/test-cold-search.ps1`: asserts index is searchable and shards are on cold node.
- `scripts/make-searchable-snapshot.ps1`: snapshots an index and restores as `remote_snapshot`; overrides cold allocation to `temp=frozen`.
- `scripts/test-frozen-search.ps1`: asserts remote snapshot index is searchable.
- `scripts/measure-index-stage.ps1`: captures resource/search metrics for one stage.
- `scripts/compare-stage-resources.ps1`: compares hot/cold/frozen metric JSON files.
...
- `promote-events-by-date.ps1` promoted `events_2026_04_23` to cold.
...
- Backfilled calendar-date indexes must be promoted explicitly because ISM age uses index creation time.
- `promote-events-by-date.ps1` moved `events_2026_05_04` and `events_2026_05_03` to cold; `events_2026_05_05` remained hot.
...
- `complete-searchable-snapshot.ps1` restored `events_2026_04_26-frozen` as `index.store.type=remote_snapshot`, validated search with 250,000 hits, then deleted original `events_2026_04_26`.
...
- `events_2026_04_07` through `events_2026_04_26` promoted to `snapshot_ready` by `promote-hot-snapshot-by-date.ps1`.
...
- Fix applied live: update policy to remove `ism_template`; delete the 20 frozen indexes; re-restore them from snapshots with `complete-searchable-snapshot.ps1` ignoring ISM policy settings.
...
- `docs/hot-searchable-snapshot-retention-10eps-report-2026-05-06.md`
...
Then compare JSON outputs:

```powershell
.\scripts\compare-stage-resources.ps1 -MetricFiles <hot.json>,<cold.json>,<frozen.json>
```
...
- Dataskope's simpler target architecture is Hot + Searchable Snapshot. Cold is optional complexity unless we need a force-merge/relocation buffer or a lower-latency intermediate tier.
...
- Searchable snapshot restore inherited cold allocation and left shards unassigned until `make-searchable-snapshot.ps1` was fixed to override allocation to `temp=frozen`.
...
- ISM policy updates do not instantly rewrite all managed index state; use `change_policy` and `lifecycle-explain.ps1`.
...
- Native `convert_index_to_remote` reached validation trouble with the local filesystem repository; OpenSearch docs require a remote repository such as S3/Azure/GCS for this action. For the no-MinIO filesystem PoC, `complete-searchable-snapshot.ps1` restored the successful snapshot with `storage_type=remote_snapshot`, validated 500,000 docs, then the source index was deleted.
...
- `docs/hot-cold-searchable-snapshot-1d-loadtest-2026-05-18.md`
...
- `scripts/reconcile-ism-policy.ps1`
...
- Real dump scripts added:
  ...
  - `scripts/run-real-retention-ingest.sh`
